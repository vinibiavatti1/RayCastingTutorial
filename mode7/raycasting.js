...
function mode7() {
    ...
    let inc = 0.9;
    let correctX = 0;
    let sin = Math.sin(degreeToRadians(data.player.angle - 45));
    let cos = Math.cos(degreeToRadians(data.player.angle - 45));
    for(let y = data.projection.halfHeight; y < data.projection.height; y++) {
        ...
        for(let x = 0; x < data.projection.width; x++) {
            ...
            _x = ((data.projection.width - correctX) * cos) - (correctX * sin);
            _y = ((data.projection.width - correctX) * sin) + (correctX * cos);
